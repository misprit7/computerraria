...
fn main() -> ! {
    ...
    loop {
        ...
        // If close snap to edge
        if ball_vel.0 < 0 && ball_pos.0 <= -ball_vel.0 as usize + 1 { ball_pos.0 = 1; ball_vel.0 *= -1 }
        if ball_vel.0 > 0 && ball_pos.0 >= WIDTH - ball_vel.0 as usize { ball_pos.0 = WIDTH-2; ball_vel.0 *= -1 }

        // Bounce
        if ball_pos.0 as i8 + ball_vel.0 < 0 {
            ball_vel.0 *= -1;
        }
        else if ball_pos.0 as i8 + ball_vel.0 >= WIDTH as i8 {
            ball_vel.0 *= -1;
        }
